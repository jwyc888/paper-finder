...
class RelationshipGraph:
    ...
    def _insert_candidate(self, a: str, b: str, descriptors: list[str], score: float) -> None:
        ...
        self.conn.execute(
            """INSERT INTO edges (src_id, dst_id, descriptors, source, status,
               ...
               ON CONFLICT(src_id, dst_id) DO UPDATE SET
                   descriptors=excluded.descriptors, confidence=excluded.confidence
               WHERE edges.status='candidate'""",
            (s, d, json.dumps(descriptors), score, _now()),
        )
        self.conn.commit()

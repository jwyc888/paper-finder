...
<body>
  <header>
    <h1>__TITLE__</h1>
    <div class="sub">Solid links are human-verified. Dashed links are inferred candidates awaiting your verdict. Hover any link for the relating descriptors.</div>
  </header>
  <div class="legend">
    <span><span class="swatch s-auth"></span><b>Authenticated</b> &mdash; verified by a human</span>
    <span><span class="swatch s-cand"></span><b>Candidate</b> &mdash; inferred from embeddings</span>
  </div>
  <div id="net"></div>
  <div class="hint">Drag nodes to rearrange. Hover edges and nodes for detail.</div>
<script>
  const G = __DATA__;
  ...
  const nodes = new vis.DataSet(G.nodes.map(n => ({
    id: n.id,
    label: n.title,
    title: "Descriptors: " + ((n.descriptors||[]).join(", ") || "none"),
    shape: "dot", size: 13,
  ...
  const edges = new vis.DataSet(G.edges.map(e => {
    ...
    const why = (e.descriptors && e.descriptors.length) ? e.descriptors.join(", ") : "(no descriptors)";
    const prov = auth ? ("verified by " + (e.verified_by || "human")) : ("inferred, score " + (e.confidence ?? "").toString().slice(0,5));
    return {
      from: e.src, to: e.dst,
      color: { color: auth ? teal : amber, opacity: auth ? 0.95 : 0.5 },
      width: auth ? 3 : 1.4,
      dashes: auth ? false : [5,5],
      title: "Related via: " + why + "  \u2014  " + prov
    };
  }));
